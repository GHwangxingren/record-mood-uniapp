	...
	<view @touchmove.stop.prevent>
		<view class="yc-fab-box ycFly" :class="{ 'yc-fab-right': !left || (left && right) }" :style="{ left: getLeft(), right: getRight(), bottom: bottom + 'rpx' }">
			<view class="yc-fab-btn" :class="{ 'yc-visible': isOpen, 'yc-fab-hidden': hidden }">
				<view class="yc-fab-item-box" :class="{ 'yc-fab-item-left': left && !right && item.imgUrl }" v-for="(item, index) in btnList" :key="index" @tap.stop="handleClick(index)">
					<view :class="[left && !right ? 'yc-text-left' : 'yc-text-right']" v-if="item.imgUrl" :style="{ fontSize: item.fontSize + 'rpx', color: item.color }">
						{{ item.text || '' }}
					</view>
					<view class="yc-fab-item" :style="{ width: width + 'rpx', height: height + 'rpx', background: item.bgColor || bgColor, borderRadius: radius }">
						<view class="yc-fab-title" v-if="!item.imgUrl && !item.share" :style="{ fontSize: item.fontSize + 'rpx', color: item.color }">{{ item.text || '' }}</view>
						<button class="yc-fab-title yc-share" v-else-if="item.share" open-type="share" plain="true">{{ item.text || '' }}</button>
