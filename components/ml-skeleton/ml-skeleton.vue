<template>
	<view class="ml-skeleton-cmomon ml-skeleton-box" :style="{width: winWidth+'px', height:winHeight+'px', backgroundColor:backgroundColor}">
		<view class="ml-skeleton-cmomon" v-for="(item,index) in skeletonElements" :key="index" :style="{width: item.width+'px', height:item.height+'px', left: item.left+'px', top: item.top+'px',backgroundColor: skeletonBgColor,borderRadius:getRadius(item.skeletonType,borderRadius)}"></view>
		<view class="ml-loading" :class="[getLoadingType(loadingType)]" v-if="isLoading"></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			//默认所有，根据页面情况进行传值
			//页面对应元素class为：ml-skeleton-rect，ml-skeleton-circular，ml-skeleton-fillet
...
<style scoped>
	...
	.ml-loading {
		...
		border-radius: 50%;
		border: 2px solid;
		animation: ml-rotate 0.7s linear infinite;
		position: fixed;
		z-index: 999999;
		left: 50%;
		top: 50%;
